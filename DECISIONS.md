...
- A failed model call returns a fixed message for its kind and logs the provider's text on the server, because that text can name the credential or the configuration. A daily cap keeps the word "daily" in its message, since the harness stops retrying on it.
