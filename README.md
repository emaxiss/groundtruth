...
The harness is not built yet. The design it will implement:
...
**Threshold calibration.** A local 3B judge is a noisy instrument. Thresholds are calibrated low on purpose, which is a statement about the judge and not about the agent. Swapping in a stronger judge is an environment change; the thresholds should be raised when that happens, and the honest version of this README will say what they were raised to and why.
...
- **The eval harness does not exist yet.** The agent is real and tested; the thing this repo is named after is still ahead of it.
- **The real-model path is under-verified.** Development has run against fake mode. The prompts are written but have not been hardened against a live model, which is exactly where guardrail prompts tend to fail.
...
- **No RAG, no persistence, no auth.** Tickets are not stored. The corpus is twelve markdown files. This is scoped as an evaluation showcase, not a support product.
